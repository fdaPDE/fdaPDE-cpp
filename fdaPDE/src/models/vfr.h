...
class FRPDE {
   ...
   public:
    ...
    FRPDE(const std::string& formula, const GeoFrame& gf, Penalty&& penalty) noexcept :
        ...
        matrix_t locs = geo_index_cast<0, POINT>(gf[0]).coordinates();
        // cumpute the nan structure
        nan_pattern_ = na_matrix(Y_);
        // initialize an empty solver
        solver_ = solver_t(gf, penalty.get());
        solver_.analyze_data(locs, vector_t::Zero(n_obs_), vector_t::Ones(n_obs_).asDiagonal());
    }
