...
template <> class fe_separable_solver<monolithic> {
   private:
   ...
   public:
    ...
    // fe_separable_solver(const GeoFrame& gf, Penalty1&& penalty1, Penalty2&& penalty2, const WeightMatrix& W) : W_(W) {
    //     fdapde_static_assert(GeoFrame::Order == 2, THIS_CLASS_IS_FOR_ORDER_TWO_GEOFRAMES_ONLY);
    // 	fdapde_assert(gf.n_layers() == 1);
    //     n_obs_ = gf[0].rows();

    //     discretize(penalty1, penalty2);
    ...
    //                 Psi__[Ns].resize(n_obs_, n_dofs_);
    //                 Psi__[Ns].setIdentity();
    //             } else {
    //                 Psi__[Ns] = point_eval_(spatial_index.coordinates());
    //             }
    //             break;
    //         }
    //         case ltype::areal: {
    ...
    //     requires(internals::is_pair_v<Penalty1> && internals::is_pair_v<Penalty2>)
    // fe_separable_solver(const GeoFrame& gf, Penalty1&& penalty1, Penalty2&& penalty2) :
    //     fe_separable_solver(gf, penalty1, penalty2, vector_t::Ones(gf[0].rows()).asDiagonal()) { }
...
// general non-parametrized separable solver factory method
template <typename... Penalty_>
    // requires(sizeof...(Penalty_) == 2 && (internals::is_valid_penalty_pair_v<Penalty_> && ...))
struct fe_separable_penalty {
    using solver_t = internals::fe_separable_solver<monolithic>;

    fe_separable_penalty(const Penalty_&... penalty) : penalty_(std::make_tuple(penalty...)) { }
...
template <typename... Penalty_>
    // requires(sizeof...(Penalty_) == 2 && (internals::is_valid_penalty_pair_v<Penalty_> && ...))
fe_separable_penalty<std::decay_t<Penalty_>...> fe_separable(const Penalty_&... penalty) {
    return fe_separable_penalty(penalty...);
}

// template <> struct fe_separable_solver<iterative> {
//    private:
//     using vector_t        = Eigen::Matrix<double, Dynamic, 1>;
//     using matrix_t        = Eigen::Matrix<double, Dynamic, Dynamic>;
//     using sparse_matrix_t = Eigen::SparseMatrix<double>;
...
//         requires(internals::is_pair_v<Penalty>)
//     fe_separable_solver(const std::string& formula, const GeoFrame& gf, Penalty&& penalty, const WeightMatrix& W) :
//         fe_separable_solver(formula, gf, penalty, W, 1e-4, 50) { }
//     template <typename GeoFrame, typename Penalty>
//         requires(internals::is_pair_v<Penalty>)
//     fe_separable_solver(const std::string& formula, const GeoFrame& gf, Penalty&& penalty) :
//         fe_separable_solver(formula, gf, penalty, Eigen::Matrix<double, Dynamic, 1>::Ones(gf[0].rows()).asDiagonal()) {
//     }
