...
// Spline-only smoother: solves (Psi^T W Psi + lambda * R1) f = Psi^T W z
struct bs_normcovmax_elliptic {
   ...
   public:
    ...
    void analyze_data(const DataLocs& locs, const matrix_t& z, const WeightMatrix& W) {
        fdapde_assert(locs.rows() > 0 && z.rows() == locs.rows() && z.cols() == 1 && W.rows() == locs.rows() && W.rows() == W.cols());
        n_obs_ = locs.rows();
        n_locs_ = n_obs_;
        n_covs_ = 0;
        eval_basis_at_(locs);
    ...
    // fit: solve (Psi^T W Psi + lambda R1) f = Psi^T W z
