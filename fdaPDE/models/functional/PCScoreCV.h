...
namespace models{
  ...
  struct ReconstructionErrorCVScore {
    ...
    double operator()(const M& m, const BlockFrame<double, int>& test) const {
      ...
      DVector<double> scores = test.get<double>(OBSERVATIONS_BLK).transpose()*m.f(); // s = Y*f
      double norm = m.f().squaredNorm() + (m.f().transpose()*m.R0()*m.f()).coeff(0,0); // \norm{f} + P(f)
      // normalize scores
      scores = scores/norm; // s/(\norm{f} + P(f))
      // return reconstruction error \norm_F(Y - s*f^T)
      return (test.get<double>(OBSERVATIONS_BLK) - m.f()*scores.transpose()).squaredNorm();
    }  
  };
