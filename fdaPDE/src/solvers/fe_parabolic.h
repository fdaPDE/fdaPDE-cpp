...
template <> class fe_parabolic_solver<direct_tag> {
   ...
   public:
    ...
    fe_parabolic_solver(const std::string& formula, const GeoFrame& gf, Penalty&& penalty, const WeightMatrix& W) :
        s_(std::get<1>(penalty)) {
        fdapde_static_assert(GeoFrame::Order == 2, THIS_CLASS_IS_FOR_ORDER_TWO_GEOFRAMES_ONLY);
	fdapde_assert(gf.n_layers() == 1);
        n_obs_ = gf[0].rows();
        n_locs_ = n_obs_;

        discretize(std::get<0>(penalty));
        analyze_data(formula, gf, W);
    }
    template <typename GeoFrame, typename Penalty>
        requires(internals::is_tuple_v<Penalty>)
    fe_parabolic_solver(const std::string& formula, const GeoFrame& gf, Penalty&& penalty) :
        fe_parabolic_solver(formula, gf, penalty, vector_t::Ones(gf[0].rows()).asDiagonal()) { }
    // construct with no data
    template <typename GeoFrame, typename WeightMatrix, typename Penalty>
        requires(internals::is_tuple_v<Penalty>)
    fe_parabolic_solver(const GeoFrame& gf, Penalty&& penalty, const WeightMatrix& W) : s_(std::get<1>(penalty)) {
        fdapde_static_assert(GeoFrame::Order == 2, THIS_CLASS_IS_FOR_ORDER_TWO_GEOFRAMES_ONLY);
	fdapde_assert(gf.n_layers() == 1);
        n_obs_ = gf[0].rows();
        n_locs_ = n_obs_;
    ...
    template <typename GeoFrame, typename Penalty>
        requires(internals::is_tuple_v<Penalty>)
    fe_parabolic_solver(const GeoFrame& gf, Penalty&& penalty) :
        fe_parabolic_solver(gf, penalty, vector_t::Ones(n_locs_).asDiagonal()) { }
...
struct fe_parabolic_penalty {
    ...
    using solver_t = std::conditional_t<
      std::is_same_v<Strategy, direct_tag>, internals::fe_parabolic_solver<direct_tag>,
      internals::fe_parabolic_solver<iterative_tag>>;
    fdapde_static_assert(
    ...
    fe_parabolic_penalty([[maybe_unused]] Strategy s, const Penalty_& penalty, const InitialCondition_& ic) :
        penalty_(std::make_tuple(penalty, ic)) { }
    const std::tuple<Penalty_, InitialCondition>& get() const { return penalty_; }
   private:
    std::tuple<Penalty_, InitialCondition> penalty_;
};
