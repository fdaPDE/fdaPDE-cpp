...
template <unsigned int N> using VectorSpaceBasis = std::vector<SVector<N>>;
...
public:
  ...
  static VectorSpaceBasis<N> orthonormalize(const VectorSpaceBasis<N>& basis);

  // project point x onto the space generated by the given orthonormal basis
  static DynamicVector projectOnto(const VectorSpaceBasis<N>& basis,
				   const SVector<N>& x);

  // project point x into the space generated by the given orthonormal basis
  static SVector<N> projectInto(const VectorSpaceBasis<N>& basis,
				const SVector<N>& x);

  // returns the euclidean distance between the point x and the space spanned by basis
  static double getL2Distance(const VectorSpaceBasis<N>& basis,
...
template <unsigned int N>
VectorSpaceBasis<N> Geometry<N>::orthonormalize(const VectorSpaceBasis<N>& basis){

  VectorSpaceBasis<N> orthonormalBasis;
...
template <unsigned int N>
DynamicVector Geometry<N>::projectOnto(const VectorSpaceBasis<N>& spaceBasis, const SVector<N> &x){
...
template <unsigned int N>
SVector<N> Geometry<N>::projectInto(const VectorSpaceBasis<N>& basis, const SVector<N> &x){
...
template <unsigned int N>
double Geometry<N>::getL2Distance(const VectorSpaceBasis<N>& basis, const SVector<N>& offset,
				  const SVector<N> &x) {
