...
public:
  ...
  static std::vector<SVector<N>> orthonormalize(const std::vector<SVector<N>>& inputBasis);

  // project point x onto the space generated by the given orthonormal basis
  static Eigen::Matrix<double, Eigen::Dynamic, 1> projectOnto(const std::vector<SVector<N>>& spaceBasis,
							      const SVector<N> &x);

  // project point x into the space generated by the given orthonormal basis
  static SVector<N> projectInto(const std::vector<SVector<N>>& basis,
				const SVector<N> &x);

  // returns the euclidean distance between the point x and the space spanned by basis
  static double getL2Distance(const std::vector<SVector<N>>& basis,
			      const SVector<N>& offset,
			      const SVector<N> &x);
};
...
template <unsigned int N>
std::vector<SVector<N>> Geometry<N>::orthonormalize(const std::vector<SVector<N>>& inputBasis){

  std::vector<SVector<N>> orthonormalBasis;
  orthonormalBasis.reserve(inputBasis.size());
  ...
  orthonormalBasis.push_back(inputBasis[0]/(inputBasis[0].norm()));

  // build orthonormal vectors following the modified gram-schmidt method
  for(int i = 1; i < inputBasis.size(); ++i){
    orthonormalBasis.push_back(inputBasis[i]);
    for(int j = 0; j < i; ++j){
      orthonormalBasis[i] -= projector(orthonormalBasis[j], inputBasis[i]);
    }
...
template <unsigned int N>
Eigen::Matrix<double, Eigen::Dynamic, 1> Geometry<N>::projectOnto(const std::vector<SVector<N>>& spaceBasis, const SVector<N> &x){
...
template <unsigned int N>
SVector<N> Geometry<N>::projectInto(const std::vector<SVector<N>>& basis, const SVector<N> &x){
...
// compute the euclidean distance from a linear subspace passing throught point offset
// this allows to cope with affine spaces (i.e. planes or lines not passing throught the origin)
template <unsigned int N>
double Geometry<N>::getL2Distance(const std::vector<SVector<N>> &basis, const SVector<N>& offset,
				  const SVector<N> &x) {
